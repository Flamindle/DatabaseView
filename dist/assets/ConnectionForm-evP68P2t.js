var e=`http://localhost:3000`;async function t(e,t={}){try{return await(await fetch(e,{...t,headers:{"Content-Type":`application/json`,...t.headers}})).json()}catch(e){return{success:!1,message:e.message}}}function n(n){return t(`${e}/get-databases`,{method:`POST`,body:JSON.stringify(n)})}function r(n){return t(`${e}/connect`,{method:`POST`,body:JSON.stringify(n)})}async function i(t){try{let n=JSON.parse(localStorage.getItem(`mysql_viewer_config`)||`{}`);if(n.dbType===`sqlite`){if(!n.dbPath)return{success:!1,message:`未连接 SQLite 数据库`}}else if(!n.database)return{success:!1,message:`未连接数据库`};let r=await(await fetch(`${e}/connect`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(n)})).json();return r.success?await(await fetch(`${e}/query-table`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({tableName:t.tableName,page:t.page||1,pageSize:t.pageSize||500,dbType:n.dbType||`mysql`})})).json():{success:!1,message:`连接数据库失败：`+r.message}}catch(e){return{success:!1,message:e.message}}}var a=`mysql_viewer_config`,o=`mysql_viewer_dashboard`;function s(e){localStorage.setItem(a,JSON.stringify(e))}function c(){let e=localStorage.getItem(a);if(e)try{return JSON.parse(e)}catch{return null}return null}function l(e){localStorage.setItem(o,JSON.stringify({version:1,...e,updatedAt:Date.now()}))}function u(){let e=localStorage.getItem(o);if(e)try{return JSON.parse(e)}catch{return null}return null}var d={},f=null,p=`mysql`;function m(e,t={}){f=t.onTablesLoaded,e.innerHTML=`
  ...
  `,d={host:document.getElementById(`host`),port:document.getElementById(`port`),user:document.getElementById(`user`),password:document.getElementById(`password`),database:document.getElementById(`database`),getDbBtn:document.getElementById(`getDbBtn`),sqliteFields:document.getElementById(`sqliteFields`),sqlitePath:document.getElementById(`sqlitePath`),sqliteBrowseBtn:document.getElementById(`sqliteBrowseBtn`),sqliteFileName:document.getElementById(`sqliteFileName`),sqliteDropZone:document.getElementById(`sqliteDropZone`),sqliteFileInput:document.getElementById(`sqliteFileInput`),connectBtn:document.getElementById(`connectBtn`),disconnectBtn:document.getElementById(`disconnectBtn`),debugBtn:document.getElementById(`debugBtn`),message:document.getElementById(`message`)},h(localStorage.getItem(`dbType`)||`mysql`),y(),g()}function h(e){p=e,localStorage.setItem(`dbType`,e),document.querySelectorAll(`.tab-btn`).forEach(t=>{t.classList.toggle(`active`,t.dataset.type===e)}),document.getElementById(`mysqlFields`).style.display=e===`mysql`?`flex`:`none`,d.sqliteFields.style.display=e===`sqlite`?`flex`:`none`,e===`mysql`?(d.sqlitePath.value=``,d.sqliteFileName.textContent=``,d.connectBtn.disabled=!d.database.value):(d.database.selectedIndex=0,d.database.disabled=!0,d.connectBtn.disabled=!d.sqlitePath.value)}function g(){document.querySelectorAll(`.tab-btn`).forEach(e=>{e.addEventListener(`click`,()=>h(e.dataset.type))}),d.getDbBtn.addEventListener(`click`,w),d.database.addEventListener(`change`,()=>{d.connectBtn.disabled=!d.database.value,b()}),d.connectBtn.addEventListener(`click`,T),d.sqliteBrowseBtn.addEventListener(`click`,()=>d.sqliteFileInput.click()),d.sqliteFileInput.addEventListener(`change`,_),d.sqlitePath.addEventListener(`input`,()=>{d.sqliteFileName.textContent=d.sqlitePath.value?d.sqlitePath.value.split(/[/\\]/).pop():``,d.connectBtn.disabled=!d.sqlitePath.value}),d.sqliteDropZone.addEventListener(`dragover`,e=>{e.preventDefault(),d.sqliteDropZone.classList.add(`dragover`)}),d.sqliteDropZone.addEventListener(`dragleave`,()=>{d.sqliteDropZone.classList.remove(`dragover`)}),d.sqliteDropZone.addEventListener(`drop`,e=>{e.preventDefault(),d.sqliteDropZone.classList.remove(`dragover`);let t=e.dataTransfer.files[0];t&&v(t)}),d.debugBtn.addEventListener(`click`,S)}function _(e){let t=e.target.files[0];t&&v(t)}function v(e){let t=e.path||e.name;d.sqlitePath.value=t,d.sqliteFileName.textContent=e.name,d.connectBtn.disabled=!1}function y(){let e=c();e&&(d.host.value=e.host||`localhost`,d.port.value=e.port||`3306`,d.user.value=e.user||`root`,d.password.value=e.password||``)}function b(){s({host:d.host.value,port:d.port.value,user:d.user.value,password:d.password.value,database:d.database.value})}var x=!1;function S(){x=!x;let e=document.getElementById(`debugPanel`);e&&(e.style.display=x?`block`:`none`)}function C(e,t=!0){d.message.textContent=e,d.message.className=`message ${t?`success`:`error`}`}async function w(){let e={host:d.host.value.trim(),port:d.port.value.trim(),user:d.user.value.trim(),password:d.password.value.trim()};if(!e.host||!e.port||!e.user){C(`请填写主机、端口、用户名`,!1);return}b();let t=await n(e);t.success?(C(t.message),d.database.innerHTML=`<option value="">请选择数据库</option>`,t.databases.forEach(e=>{let t=document.createElement(`option`);t.value=e,t.textContent=e,d.database.appendChild(t)}),d.database.disabled=!1):C(t.message,!1)}async function T(){p===`mysql`?await E():await D()}async function E(){let e=d.database.value.trim();if(!e){C(`请选择数据库`,!1);return}let t={dbType:`mysql`,host:d.host.value.trim(),port:d.port.value.trim(),user:d.user.value.trim(),password:d.password.value.trim(),database:e};b();let n=await r(t);n.success?(C(n.message),d.connectBtn.disabled=!0,d.disconnectBtn.disabled=!1,window.dispatchEvent(new CustomEvent(`dbTablesLoaded`,{detail:{tables:n.tables,dbType:`mysql`}})),f&&f(n.tables)):C(n.message,!1)}async function D(){let e=d.sqlitePath.value.trim();if(!e){C(`请选择 SQLite 数据库文件`,!1);return}let t=await r({dbType:`sqlite`,dbPath:e});t.success?(C(t.message),d.connectBtn.disabled=!0,d.disconnectBtn.disabled=!1,window.dispatchEvent(new CustomEvent(`dbTablesLoaded`,{detail:{tables:t.tables,dbType:`sqlite`}})),f&&f(t.tables)):C(t.message,!1)}export{l as a,s as i,c as n,i as o,u as r,m as t};